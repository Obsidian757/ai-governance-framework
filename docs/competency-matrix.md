# 12th House AI - Competency Matrix

**Document Control**
- Version: 1.0
- Effective Date: February 1, 2026
- Owner: Brannon Solomon (Founder & CEO)
- Next Review: February 1, 2027

---

## 1. Purpose

This competency matrix documents the skills, qualifications, and expertise required for AI-related roles at 12th House AI, demonstrating compliance with ISO/IEC 42001:2023 (Clause 7.2) and federal contractor requirements.

---

## 2. Personnel Competency Record

### Brannon Joseph Solomon Jr. - Founder & CEO

**Role Coverage:** Leadership, AI Risk Owner, Technical Lead, Compliance Officer

#### 2.1 Formal Education

| Qualification | Institution | Year | Relevance to AI Work |
|---------------|-------------|------|---------------------|
| **Master's Degree** | Regent University | 2015 | Management, strategic planning, research methods |
| **Bachelor's Degree** | Regent University | 2013 | Technical foundation |

#### 2.2 Professional Experience

| Role | Organization | Duration | AI/Compliance Relevance |
|------|--------------|----------|------------------------|
| Senior Systems Engineer | NextGen Federal Systems | 2024-2026 | Federal AI/ML solutions, NIST compliance |
| IT Professional | Regent University | 2015-2019 | Enterprise IT, systems administration |
| Systems Technician / E6 | U.S. Navy | 20+ years | Mission-critical systems, security protocols, leadership |

**Total Years of Experience:** 20+ years in technology and systems  
**Federal Contracting Experience:** 10+ years

#### 2.3 Security Credentials

| Credential | Status | Expiration | Verification |
|------------|--------|------------|--------------|
| **DoD Secret Clearance** | Active | 2030 | eQIP verified |
| Background Investigation | Current | N/A | Federal contractor standard |

#### 2.4 Technical Competencies

| Competency Area | Proficiency | Evidence |
|-----------------|-------------|----------|
| **NIST AI Risk Management Framework** | Practitioner | Implementation work performed at prior federal employer; documentation created at 12th House AI |
| **OMB Memoranda (M-25-21, M-25-22)** | Practitioner | Published ISO ↔ federal compliance mapping; ongoing federal-policy tracking |
| **ISO/IEC 42001:2023** | Implementing | Organizational certification in progress (Stage 2 audit scheduled Feb 2026); full AIMS documentation set authored |
| **Virginia EO-30 / VITA / EA-225** | Practitioner | Published ISO ↔ Commonwealth of Virginia control mapping |
| **NIST SP 800-171** | Practitioner | CUI-protection work performed at prior federal employer |
| **DFARS 252.204-7012** | Knowledgeable | Defense-contractor flow-down review |
| **CMMC Framework** | Knowledgeable | Awareness only; **not** a CMMC Registered Practitioner and not currently pursuing |
| **FedRAMP / IL5** | Knowledgeable | Aware; no current production deployments under 12th House AI |
| **HIPAA** | Knowledgeable | Compliance awareness |
| **FERPA** | Knowledgeable | Education-sector awareness (relevant to Virginia AI Education Guidelines work) |
| **DoD AI Ethical Principles** | Practitioner | Practitioner-level familiarity through prior federal-employer work |

#### 2.5 AI/ML Technical Skills

| Technology | Proficiency | Evidence |
|------------|-------------|----------|
| **Large Language Models (Claude, GPT, Kimi K2.5)** | Practitioner | Daily use, prompt engineering, API integration |
| **Voice AI (ElevenLabs, Vapi)** | Familiar | Hands-on experimentation |
| **Workflow Automation (n8n, Zapier)** | Familiar | Personal automation projects |
| **Cloud AI (Azure AI, Azure GovCloud)** | Familiar | Awareness from prior federal-employer work |
| **Python** | Competent | Scripting, automation, AI integration |
| **API Integration** | Practitioner | Multiple AI service integrations |
| **RAG (Retrieval-Augmented Generation)** | Practitioner | See companion repo `federal-ai-policy-rag` for a published RAG implementation |

#### 2.6 Business Competencies

| Competency | Proficiency | Evidence |
|------------|-------------|----------|
| **Federal Contracting** | Practitioner | Prior federal contractor employment (W-2); 8(a) status to be verified; SAM.gov registration status to be verified per current state |
| **Strategic Consulting** | Expert | 5% Framework, Mission Planning Framework developed |
| **Executive Engagement** | Proficient | C-level client interactions, executive training delivery |
| **Technical Writing** | Expert | Documentation, proposals, frameworks |
| **Project Management** | Proficient | 90-day implementation methodology |

---

## 3. Role Competency Requirements

### 3.1 Leadership (CEO/CAIO equivalent)

**Minimum Requirements:**
- [ ] 10+ years technology leadership experience
- [ ] Federal contracting experience
- [ ] Understanding of AI ethics and governance
- [ ] Business strategy and P&L responsibility

**Assessed Against:**
| Requirement | Brannon Solomon | Status |
|-------------|-----------------|--------|
| 10+ years technology | 20+ years | ✅ Exceeds |
| Federal contracting | 10+ years | ✅ Exceeds |
| AI ethics/governance | Extensive knowledge | ✅ Meets |
| Business strategy | Founder/CEO | ✅ Meets |

---

### 3.2 AI Risk Owner

**Minimum Requirements:**
- [ ] Understanding of AI risk categories (bias, privacy, security, reliability)
- [ ] Knowledge of risk assessment methodologies
- [ ] Familiarity with NIST AI RMF
- [ ] Ability to evaluate AI impact

**Assessed Against:**
| Requirement | Brannon Solomon | Status |
|-------------|-----------------|--------|
| AI risk categories | Practitioner-level familiarity | ✅ Meets |
| Risk assessment | NIST RMF implementation work (prior employer) | ✅ Meets |
| NIST AI RMF | Practitioner | ✅ Meets |
| AI impact evaluation | Documented impact-assessment templates published | ✅ Meets |

---

### 3.3 Technical Lead

**Minimum Requirements:**
- [ ] Hands-on AI/ML development experience
- [ ] Understanding of AI lifecycle management
- [ ] Security and privacy technical controls knowledge
- [ ] Third-party AI integration experience

**Assessed Against:**
| Requirement | Brannon Solomon | Status |
|-------------|-----------------|--------|
| AI/ML development | VoiceGuard, RMF Navigator | ✅ Meets |
| Lifecycle management | Documented process | ✅ Meets |
| Security/privacy controls | NIST 800-171 expert | ✅ Exceeds |
| Third-party AI | Multiple vendor integrations | ✅ Exceeds |

---

### 3.4 Compliance Officer

**Minimum Requirements:**
- [ ] Knowledge of applicable regulations (federal, industry-specific)
- [ ] Audit experience or awareness
- [ ] Documentation and record-keeping skills
- [ ] Continuous learning commitment

**Assessed Against:**
| Requirement | Brannon Solomon | Status |
|-------------|-----------------|--------|
| Regulatory knowledge | OMB, NIST, Virginia EO-30 / VITA / EA-225, DFARS, HIPAA awareness | ✅ Meets |
| Audit experience | Internal-audit experience as part of 12th House AIMS preparation; no external client audits performed under 12th House yet | ✅ Meets (in-progress evidence) |
| Documentation | Comprehensive AIMS package, federal + Virginia control mappings | ✅ Exceeds |
| Continuous learning | Ongoing federal + Virginia policy tracking | ✅ Meets |

---

## 4. Competency Gap Analysis

### 4.1 Current Gaps

| Gap Area | Current Level | Target Level | Gap Closure Plan | Timeline |
|----------|---------------|--------------|------------------|----------|
| ISO 42001 formal certification | Self-taught | Certified | Pursue PECB Lead Implementer cert | Q2 2026 |
| HIPAA implementation | Knowledge | Expert | HIPAA training course | Q2 2026 |
| Advanced ML/deep learning | Aware | Competent | Self-study, courses | Q3 2026 |

### 4.2 Planned Certifications

| Certification | Target Date | Status |
|---------------|-------------|--------|
| ISO/IEC 42001:2023 (Organizational) | Feb 28, 2026 | In progress |
| PECB ISO 42001 Lead Implementer | Q2 2026 | Under consideration |

12th House AI is **not** currently pursuing CMMC Registered Practitioner (RP) certification or CISM. These were on an earlier strategy draft and have been withdrawn from the active roadmap.

---

## 5. Training Records

### 5.1 Formal Training Log

| Training | Provider | Date | Hours | Verification |
|----------|----------|------|-------|--------------|
| NIST AI RMF Overview | NIST Self-Study | 2024 | 8 | Certificate |
| Claude API & Best Practices | Anthropic | 2024 | 4 | Completion |
| ElevenLabs Voice AI | ElevenLabs | 2024 | 4 | Completion |
| Azure AI Fundamentals | Microsoft | 2023 | 8 | Certificate |
| Federal Contracting | SBA | 2023 | 16 | Certificate |
| 8(a) Business Development | SBA | 2023 | 8 | Certificate |
| Cybersecurity Awareness | DoD (annual) | 2025 | 2 | Completion |
| DFARS/CMMC Requirements | Industry | 2024 | 8 | Attendance |

### 5.2 Self-Directed Learning (2025-2026)

| Topic | Method | Estimated Hours | Evidence |
|-------|--------|-----------------|----------|
| ISO/IEC 42001:2023 standard review | Document study | 10 | Notes, documentation created |
| OMB M-25-21/M-25-22 analysis | Regulatory review | 8 | Compliance mapping guide |
| EO 14110 / EO 14179 | Regulatory review | 6 | Analysis documents |
| Voice AI development (VoiceGuard) | Hands-on project | 100+ | Working product |
| Claude Code / MCP Servers | Practical application | 40+ | Configurations, workflows |
| AI compliance frameworks | Research + synthesis | 50+ | Client deliverables |

**Total Self-Directed Learning (2025-2026):** 214+ hours

### 5.3 Annual Training Plan

| Topic | Due Date | Status |
|-------|----------|--------|
| Cybersecurity Awareness (DoD) | Dec 2026 | Planned |
| Privacy/Data Protection | Q1 2026 | Planned |
| ISO 42001 Implementation | Q1 2026 | In progress |
| HIPAA for AI | Q2 2026 | Planned |
| Bias/Fairness in AI | Q2 2026 | Planned |

---

## 6. Evidence Repository

Competency evidence is maintained as follows:
- Founder credentials (clearance, education, prior employment): retained by founder
- LinkedIn profile: education and work-history record
- Published artifacts in this repository: AIMS documentation, ISO ↔ federal control mapping, ISO ↔ Virginia EO-30 / VITA / EA-225 control mapping
- SAM.gov registration: status to be verified per current state
- Client testimonials / references: 12th House AI has no completed client engagements to reference at this time. This section will be updated when engagements complete.

---

## 7. Contractor/Partner Competency Requirements

### 7.1 Minimum Requirements for AI Work

Any contractors or partners working on AI projects must demonstrate:

| Requirement | Verification Method |
|-------------|---------------------|
| Relevant technical skills for assigned work | Resume review, portfolio |
| Understanding of data protection | Training completion or assessment |
| Signed confidentiality agreement | NDA on file |
| Security clearance (if CUI/federal work) | Clearance verification |
| ISO 42001/NIST awareness | Training or assessment |

### 7.2 Onboarding Checklist

- [ ] Resume/CV reviewed and approved
- [ ] NDA executed
- [ ] Data handling policy acknowledged
- [ ] Access provisioned (minimum necessary)
- [ ] Clearance verified (if applicable)
- [ ] Training assigned (AI governance basics)
- [ ] Competency record created

---

## 8. Competency Review

### 8.1 Annual Review Process

- Review all personnel competency records annually
- Identify gaps against role requirements
- Document training completed in past year
- Set training objectives for coming year
- Update competency matrix

### 8.2 Continuous Improvement

- Track emerging AI skills/certifications
- Monitor regulatory changes requiring new knowledge
- Add new competencies as business evolves
- Document lessons learned from projects

---

**Competency Assessment Attestation:**

I attest that the competency information documented above is accurate and current.

**Brannon Joseph Solomon Jr.**  
Founder & CEO, 12th House AI  
Date: ________________

---

**Document Revision History:**

| Version | Date | Author | Changes |
|---------|------|--------|---------|
| 1.0 | Feb 1, 2026 | Brannon Solomon | Initial competency matrix for ISO 42001 |
